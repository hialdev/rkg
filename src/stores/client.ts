...
interface ClientState {
   ...
   add: ({ data }: { data: ClientData }) => Promise<any>;
   update: ({ id, data }: { id: string; data: ClientData }) => Promise<any>;
      ...
      (set, get) => ({
         ...
         all: async (params?: any) => {
            ...
            if (response.data.success && response.data.data) {
               set({ clients: response.data.data.clients || response.data.data });
            }
            return response.data;
         },
         detail: async ({ id }) => {
            const response = await protectedApi.get(`/clients/${id}`);
            return response.data;
         },
         add: async ({ data }) => {
            let payload: ClientData | FormData = data;
            let config = {};
            ...
            const response = await protectedApi.post(`/clients`, payload, config);
            return response.data;
         },
         update: async ({ id, data }) => {
            ...
            const response = await protectedApi.post(`/clients/${id}`, payload, config);
            return response.data;
         },
         delete: async ({ id }) => {
            const response = await protectedApi.delete(`/clients/${id}`);
            return response.data;
         },
      }),
      {
         name: 'client-store', // key di localStorage
         partialize: (state) => ({
