...
import { FaqTableRow } from './components/faq-table-row';
...
export function FaqListView() {
   ...
   const { all, delete: destroy } = useFaqStore();

   const [tableData, setTableData] = useState<FaqData[]>([]);
   ...
   const filters = useSetState<IFaqTableFilters>({ title: '' });
   ...
   const fetchData = async () => {
      ...
      try {
         ...
         if (res.success) {
            const { pagination: pgnt, faqs } = res.data;
            setTableData(faqs || []);
            setPagination({
               page: pgnt.page,
               limit: pgnt.limit,
      ...
      try {
         ...
         for (const id of table.selected) {
            try {
               const reqdel = await destroy({ id });
               if (reqdel.success) {
                  toast.success(reqdel.message || `Berhasil hapus faq dengan ID: ${id}`);
               } else {
                  toast.error(reqdel.message || `Gagal hapus faq dengan ID: ${id}`);
               }
            } catch (error) {
               toast.error(`Gagal hapus faq dengan ID: ${id}`);
            }
         }
         ...
         <DashboardContent>
            <CustomBreadcrumbs
               ...
               links={[
                  ...
                  { name: 'FAQs', href: paths.dashboard.faqs.root },
                  { name: 'List' },
               ]}
               action={
                  <Button
                           ...
                           <TableBody>
                              {dataFiltered.map((row) => (
                                 <FaqTableRow
                                    ...
                                    editHref={
                                       row.id ? paths.dashboard.faqs.edit(row.id) : ''
                                    }
                                 />
                              ))}
...
function applyFilter({ inputData, comparator, filters }: ApplyFilterProps) {
   ...
   if (title) {
      inputData = inputData.filter(
         (faq) =>
            (faq.title ?? '').toLowerCase().includes(title.toLowerCase()) ||
            (faq.content ?? '').toLowerCase().includes(title.toLowerCase())
      );
   }
