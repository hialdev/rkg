...
import { EventTypeType, EventTypeSchema } from 'src/types/event-type';

// ----------------------------------------------------------------------

type Props = {
   onSuccess?: () => void;
   currentEventType?: EventTypeType;
};

export function EventTypeForm({ currentEventType, onSuccess }: Props) {
   const router = useRouter();
   const [slug, setSlug] = useState<string>(currentEventType?.slug ?? '');

   let parsedGalleries: any[] = [];

   if (typeof currentEventType?.galleries === 'string' && currentEventType.galleries !== '') {
      try {
         parsedGalleries = JSON.parse(currentEventType.galleries);
      } catch (err) {
   ...
   const defaultValues: EventTypeType = {
      title: currentEventType?.title || '',
      slug: currentEventType?.slug || '',
      description: currentEventType?.description || '',
      image:
         typeof currentEventType?.image === 'string' && currentEventType?.image
            ? `${CONFIG.apiHostUrl}/${currentEventType.image}`
            : currentEventType?.image,
      content: currentEventType?.content || '',
      galleries:
   ...
   const onSubmit = handleSubmit(async (data) => {
      try {
         ...
         if (currentEventType?.id) {
            ...
            result = await update({ id: currentEventType.id, data: formData });
         } else {
         ...
         if (result.success) {
            ...
            onSuccess?.();
            reset();
            router.push(paths.dashboard.event_types.root);
         } else {
            toast.error(result.message || 'An error occurred while saving the event type');
         }
      } catch (error) {
         console.error(error);
      ...
      <Form methods={methods} onSubmit={onSubmit}>
         ...
         <Box sx={{ position: 'sticky', bottom: 0, end: 0, start: 0, zIndex: 99, pb: 2 }}>
            <Button type="submit" variant="contained" fullWidth loading={isSubmitting}>
               {currentEventType ? 'Update' : 'Add New'}
            </Button>
         </Box>
      </Form>
   );
