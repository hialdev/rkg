...
import { EventServiceTableRow } from './event-service-table-row';

// ----------------------------------------------------------------------

type Props = {
  eventServices: any[];
  onEdit: (eventService: any) => void;
  onDelete: () => void;
...
export function EventServiceTable({ eventServices, onEdit, onDelete, editHref }: Props) {
  ...
  const dataFiltered = eventServices.sort(getComparator(order, orderBy));
  ...
  const handleDeleteRow = (id: string) => {
    const deleteRow = eventServices.filter((item) => item.id !== id);
    // In a real app, you would call a delete API endpoint here
    onDelete();
  };

  const handleDeleteSelected = () => {
    const deleteRows = eventServices.filter((item) => !selected.includes(item.id));
