...
import { TablesCUForm } from './forms/tables-cu-form';
import { TablesTableRow } from './components/tables-table-row';
import { TablesTableToolbar } from './components/tables-table-toolbar';
import { TablesTableFiltersResult } from './components/tables-table-filters-result';
...
const TABLE_HEAD: TableHeadCellProps[] = [
   ...
   { id: 'generate_widget', label: 'Generate Widget', width: 180 },
   { id: 'fields', label: 'Fields', width: 180 },
   { id: '', width: 88 },
];
...
export function TablesListView() {
   ...
   const { all, delete: destroy } = useTabletore();
   ...
   const filters = useSetState<any>({ name: '' });
   ...
   const fetchData = async () => {
      ...
      if (res.success) {
         const { pagination:pgnt, tables } = res.data;
         setTableData(tables || []);
         setPagination({
            page: pgnt.page,
            limit: pgnt.limit,
         ...
         toast.error('Gagal memuat data tables');
      }
